...
export class WishlistService {
  ...
  async create(wishlistDto: CreateWishlistDto): Promise<WishlistEntity> {
    ...
    try{
        const wishlist: WishlistEntity = await this.wishlistRepo.create({ user_id, announcement_id })
  ...
  async findAllWishlist(): Promise<WishlistEntity[]>{  //MODIFIER POUR TROUVER LA WISHLIST D'UN USER 
